...
- (void) readjustLabels
{
	...
	if (label[0].frame.size.width > self.frame.size.width){
        ...
        if(label[0].textAlignment == UITextAlignmentCenter){
            // Center this label
            CGPoint center;
        ...
        }else if(label[0].textAlignment == UITextAlignmentRight){
            frame.origin.x = self.frame.size.width - label[0].frame.size.width;
            frame.origin.y = 0;
            label[0].frame = frame;
            
        }else if(label[0].textAlignment == UITextAlignmentLeft){
            frame.origin.x = 0;
            frame.origin.y = 0;
            label[0].frame = frame;
        }
...
{
    for (int i=0; i<NUM_LABELS; ++i){
		label[i].textAlignment = textAlignment;
	}
	[self readjustLabels];
}
